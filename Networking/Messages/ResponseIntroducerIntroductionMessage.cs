// Decompiled with JetBrains decompiler
// Type: SKSSL.Networking.Messages.ResponseIntroducerIntroductionMessage
// Assembly: SKSSL, Version=1.0.0.0, Culture=neutral, PublicKeyToken=null
// MVID: 8B98CDD1-E58F-4F79-8D54-EFB305228BC6
// Assembly location: D:\DEVBOOK_6\DECOMP\CS\SKSSL.dll

using System.Net;

#nullable enable
namespace SKSSL.Networking.Messages
{
  public class ResponseIntroducerIntroductionMessage : MessageBase
  {
    public IPEndPoint InternalPeerEndPoint { get; set; }

    public IPEndPoint ExternalPeerEndPoint { get; set; }

    public ResponseIntroducerIntroductionMessage()
      : base(MessageType.ResponseIntroducerIntroduction)
    {
    }

    public override void WritePayload(IValueWriter writer)
    {
      base.WritePayload(writer);
      writer.WriteBytes(this.InternalPeerEndPoint.Address.GetAddressBytes());
      writer.WriteInt32(this.InternalPeerEndPoint.Port);
      writer.WriteBytes(this.ExternalPeerEndPoint.Address.GetAddressBytes());
      writer.WriteInt32(this.ExternalPeerEndPoint.Port);
    }

    public override void ReadPayload(IValueReader reader)
    {
      base.ReadPayload(reader);
      this.InternalPeerEndPoint = new IPEndPoint(new IPAddress(reader.ReadBytes()), reader.ReadInt32());
      this.ExternalPeerEndPoint = new IPEndPoint(new IPAddress(reader.ReadBytes()), reader.ReadInt32());
    }
  }
}
